...
import { deleteDeviceApi, getDevicesApi } from "../../api/deviceApi";

export default function useManageDevice() {
  ...
  const filteredQuery = data.filter((item) => {
    ...
    return (
      ...
      item.stock.toLowerCase().includes(lowerCaseQuery) ||
  ...
  const onDelete = async (itemId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await deleteDeviceApi(itemId);
  ...
  const exportPdf = () => {
    import("jspdf").then((jsPDF) => {
      import("jspdf-autotable").then(() => {
        ...
        doc.setFontSize(18);
        doc.text("Danh sách thiết bị", 14, 22);
        ...
        doc.save("danh-sach-thiet-bi.pdf");
      });
    });
  };
  ...
  const exportExcel = () => {
    import("xlsx").then((xlsx) => {
      ...
      saveAsExcelFile(excelBuffer, "danh-sach-thiet-bi");
    });
  };
