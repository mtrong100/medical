...
export default function useManageAppointment() {
  ...
  const onDelete = async (itemId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await deleteAppointmentApi(itemId);
  ...
  const exportPdf = () => {
    import("jspdf").then((jsPDF) => {
      import("jspdf-autotable").then(() => {
        ...
        doc.setFontSize(18);
        doc.text("Danh sách lịch khám bệnh", 14, 22);
        ...
        doc.save("danh-sach-lich-kham-benh.pdf");
      });
    });
  };
  ...
  const exportExcel = () => {
    import("xlsx").then((xlsx) => {
      ...
      saveAsExcelFile(excelBuffer, "danh-sach-lich-kham-benh");
    });
  };
