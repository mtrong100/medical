...
const UpdateMedicineModal = ({
  ...
  useEffect(() => {
    if (updateVal) {
      reset({ ...updateVal });
      setSelectedCategory(updateVal.category?._id);
      setSelectedUnit(updateVal.unit);
    }
  }, [reset, updateVal]);

  const handleUpdateMedicine = async (values) => {
    try {
      const body = {
        ...values,
        unit: selectedUnit,
      ...
      await updateMedicineApi(updateVal._id, body);
  ...
  return (
    <>
      ...
      <Dialog
        header={`Cập nhật thuốc ${updateVal?.name}`}
        visible={visible2}
        style={{ width: "30vw" }}
        onHide={() => {
          if (!visible2) return;
